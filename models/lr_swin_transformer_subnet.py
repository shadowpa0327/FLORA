...
class WindowAttention(nn.Module):
    ...
    def __init__(self, ratio, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, svd_config, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        self.attn = WindowAttention(
            svd_config[0], dim, window_size=to_2tuple(self.window_size), num_heads=num_heads,
            qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop)

        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = Mlp([svd_config[1], svd_config[2]],in_features=dim, hidden_features=mlp_hidden_dim,
    ...
    def flops(self):
        ...
        flops += H * W * min(self.dim * self.dim * self.mlp_ratio,
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, svd_config, dim, input_resolution, depth, num_heads, window_size,
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(svd_config=svd_config[i], dim=dim, input_resolution=input_resolution,
...
class LRSwinTransformer(nn.Module):
    r""" Swin Transformer
