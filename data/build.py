...
def build_dataset(is_train, config):
    ...
    if config.DATA.DATASET == 'imagenet':
        ...
        if prefix == 'val':
            step = int(1/config.DATA.VAL_PERCENTAGE)
            dataset = torch.utils.data.Subset(dataset, torch.tensor([i for i in range(len(dataset))])[::step])
        elif prefix == 'train':
            step = int(1/config.DATA.TRAIN_PERCENTAGE)
            dataset = torch.utils.data.Subset(dataset, torch.tensor([i for i in range(len(dataset))])[::step])
            
        nb_classes = 1000
    elif config.DATA.DATASET == 'cifar100' :
        dataset = datasets.CIFAR100(config.DATA.DATA_PATH, train=is_train, transform=transform)
        nb_classes = 100
    else:
        raise NotImplementedError("We only support ImageNet, CIFAR-100 Now.")
...
def build_transform(is_train, config):
    ...
    if resize_im:
        if config.TEST.CROP:
            size = int(config.DATA.IMG_SIZE / 0.9)
            t.append(
